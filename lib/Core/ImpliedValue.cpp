...
void ImpliedValue::getImpliedValues(ref<Expr> e, ref<ConstantExpr> value, ImpliedValueList &results) {
    switch (e->getKind()) {
        case Expr::Constant: {
            assert(value == cast<ConstantExpr>(e) && "error in implied value calculation");
            break;
        }
        ...
        case Expr::Select: {
            ...
            if (ConstantExpr *TrueCE = dyn_cast<ConstantExpr>(se->getTrue())) {
                if (ConstantExpr *FalseCE = dyn_cast<ConstantExpr>(se->getFalse())) {
                    if (TrueCE != FalseCE) {
                        if (value == TrueCE) {
                            getImpliedValues(se->getCondition(), ConstantExpr::alloc(1, Expr::Bool), results);
                        } else {
                            assert(value == FalseCE && "err in implied value calculation");
                            getImpliedValues(se->getCondition(), ConstantExpr::alloc(0, Expr::Bool), results);
                        }
                    }
                }
            }
        ...
        case Expr::Concat: {
            ...
            getImpliedValues(ce->getKid(0), value->Extract(ce->getKid(1)->getWidth(), ce->getKid(0)->getWidth()),
                             results);
            getImpliedValues(ce->getKid(1), value->Extract(0, ce->getKid(1)->getWidth()), results);
            break;
        }
        ...
        case Expr::SExt: {
            ...
            getImpliedValues(ce->getSrc(), value->Extract(0, ce->getSrc()->getWidth()), results);
            break;
        }
...
void ImpliedValue::checkForImpliedValues(Solver *S, ref<Expr> e, ref<ConstantExpr> value) {
    ...
    for (ImpliedValueList::iterator i = results.begin(), ie = results.end(); i != ie; ++i) {
        std::map<ref<ReadExpr>, ref<ConstantExpr>>::iterator it = found.find(i->first);
        if (it != found.end()) {
    ...
    for (std::vector<ref<ReadExpr>>::iterator i = reads.begin(), ie = reads.end(); i != ie; ++i) {
        ReadExpr *re = i->get();
        assumption.push_back(UltExpr::create(re->getIndex(), ConstantExpr::alloc(re->getUpdates().getRoot()->getSize(),
