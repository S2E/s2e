...
class PCLoggingSolver : public SolverImpl {
    ...
    void startQuery(const Query &query, const char *typeName, const ref<Expr> *evalExprsBegin = 0,
                    const ref<Expr> *evalExprsEnd = 0, const Array *const *evalArraysBegin = 0,
        ...
        printer->printQuery(ros, query.constraints, query.expr, evalExprsBegin, evalExprsEnd, evalArraysBegin,
                            evalArraysEnd);
        ros.flush();
...
public:
    PCLoggingSolver(Solver *_solver, std::string path)
        : solver(_solver), os(path.c_str(), std::ios::trunc), printer(ExprPPrinter::create(os)), queryCount(0) {
    }
    ~PCLoggingSolver() {
        delete printer;
        delete solver;
    ...
    bool computeInitialValues(const Query &query, const std::vector<const Array *> &objects,
        ...
        if (objects.empty()) {
            ...
            startQuery(query, "InitialValues", 0, 0, &objects[0], &objects[0] + objects.size());
        }
