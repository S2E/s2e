    ...
    for (i = 0; test_cases[i].json_in; i++) {
        for (j = 0; j < 2; j++) {
            if (test_cases[i].utf8_out) {
                ...
                g_assert_cmpstr(qstring_get_str(cstr), ==, test_cases[i].utf8_out);
...
static void string_with_quotes(void) {
    const char *test_cases[] = {"\"the bee's knees\"", "'double quote \"'", NULL};
    int i;
    QString *str;
    char *cstr;

    for (i = 0; test_cases[i]; i++) {
        str = qobject_to(QString, qobject_from_json(test_cases[i], &error_abort));
        g_assert(str);
...
static void utf8_string(void) {
    ...
    static const struct {
        ...
        const char *json_out;
    } test_cases[] = {/* 0  Control characters */
                      {
                          /*
                      ...
                      /* 1  Some correct UTF-8 text */
                      {
                          /* a bit of German */
                          "Falsches \xC3\x9C"
                          "ben von Xylophonmusik qu\xC3\xA4lt"
                          " jeden gr\xC3\xB6\xC3\x9F"
                          "eren Zwerg.",
                          "Falsches \xC3\x9C"
                          "ben von Xylophonmusik qu\xC3\xA4lt"
                          " jeden gr\xC3\xB6\xC3\x9F"
                      ...
                      {/* last one in last plane: U+10FFFD */
                       "\xF4\x8F\xBF\xBD", "\xF4\x8F\xBF\xBD", "\\uDBFF\\uDFFD"},
                      {
...
static void int_number(void) {
    ...
    for (i = 0; test_cases[i].encoded; i++) {
        qnum = qobject_to(QNum, qobject_from_json(test_cases[i].encoded, &error_abort));
        g_assert(qnum);
        ...
        g_assert_cmpfloat(qnum_get_double(qnum), ==, (double) test_cases[i].decoded);

        str = qobject_to_json(QOBJECT(qnum));
        g_assert_cmpstr(str->str, ==, test_cases[i].reencoded ?: test_cases[i].encoded);
...
static void simple_dict(void) {
    ...
    struct {
    ...
    } test_cases[] = {
        {
            ...
            .decoded = QLIT_QDICT(((QLitDictEntry[]){{"foo", QLIT_QNUM(42)}, {"bar", QLIT_QSTR("hello world")}, {}})),
        },
        {
            .encoded = "{}",
        ...
        {
            .encoded = "{\"foo\": 43}",
            .decoded = QLIT_QDICT(((QLitDictEntry[]){{"foo", QLIT_QNUM(43)}, {}})),
        },
        {}};
 ...
 *          }
 */
static void gen_test_json(GString *gstr, int nest_level_max, int elem_count) {
    int i;
...
static void simple_list(void) {
    ...
    struct {
        ...
        QLitObject decoded;
                          ...
                          .encoded = "[43,42]",
                          .decoded = QLIT_QLIST(((QLitObject[]){QLIT_QNUM(43), QLIT_QNUM(42), {}})),
                      },
                      {
                          .encoded = "[43]",
                          .decoded = QLIT_QLIST(((QLitObject[]){QLIT_QNUM(43), {}})),
                      },
                      {
                          .encoded = "[]",
...
static void simple_whitespace(void) {
    ...
    struct {
        ...
        },
        {
            .encoded = "\t[ 43 , { 'h' : 'b' },\r\n\t[ ], 42 ]\n",
            .decoded = QLIT_QLIST(((QLitObject[]){QLIT_QNUM(43),
                                                  QLIT_QDICT(((QLitDictEntry[]){{"h", QLIT_QSTR("b")}, {}})),
        ...
        },
        {
            ...
            .decoded = QLIT_QLIST(
                ((QLitObject[]){QLIT_QNUM(43),
                                QLIT_QDICT(((QLitDictEntry[]){{"h", QLIT_QSTR("b")}, {"a", QLIT_QNUM(32)}, {}})),
                                QLIT_QLIST(((QLitObject[]){{}})),
                                QLIT_QNUM(42),
                                {}})),
        },
...
static void simple_interpolation(void) {
    ...
    QLitObject decoded = QLIT_QLIST(((QLitObject[]){
        QLIT_QNUM(1), QLIT_QSTR("100%"), QLIT_QLIST(((QLitObject[]){QLIT_QNUM(32), QLIT_QNUM(42), {}})), {}}));
