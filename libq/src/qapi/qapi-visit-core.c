...
bool visit_policy_reject(Visitor *v, const char *name, unsigned special_features, Error **errp) {
...
bool visit_policy_skip(Visitor *v, const char *name, unsigned special_features) {
...
static bool visit_type_uintN(Visitor *v, uint64_t *obj, const char *name, uint64_t max, const char *type,
                             Error **errp) {
    uint64_t value = *obj;
    ...
    if (value > max) {
        assert(v->type == VISITOR_INPUT);
        error_setg(errp, QERR_INVALID_PARAMETER_VALUE, name ? name : "null", type);
        return false;
    }
    *obj = value;
    return true;
...
static bool visit_type_intN(Visitor *v, int64_t *obj, const char *name, int64_t min, int64_t max, const char *type,
                            Error **errp) {
    int64_t value = *obj;
...
bool visit_type_int16(Visitor *v, const char *name, int16_t *obj, Error **errp) {
    ...
    value = *obj;
    ok = visit_type_intN(v, &value, name, INT16_MIN, INT16_MAX, "int16_t", errp);
    *obj = value;
    return ok;
}
...
bool visit_type_int32(Visitor *v, const char *name, int32_t *obj, Error **errp) {
    ...
    value = *obj;
    ok = visit_type_intN(v, &value, name, INT32_MIN, INT32_MAX, "int32_t", errp);
    *obj = value;
    return ok;
}
...
static bool output_type_enum(Visitor *v, const char *name, int *obj, const QEnumLookup *lookup, Error **errp) {
    int value = *obj;
    char *enum_str;
...
static bool input_type_enum(Visitor *v, const char *name, int *obj, const QEnumLookup *lookup, Error **errp) {
    int64_t value;
    ...
    if (value < 0) {
        error_setg(errp, "Parameter '%s' does not accept value '%s'", name ? name : "null", enum_str);
        return false;
    }
