...
#define qobject_output_add(qov, name, value)  qobject_output_add_obj(qov, name, QOBJECT(value))
#define qobject_output_push(qov, value, qapi) qobject_output_push_obj(qov, QOBJECT(value), qapi)
...
static void qobject_output_push_obj(QObjectOutputVisitor *qov, QObject *value, void *qapi) {
...
static void qobject_output_add_obj(QObjectOutputVisitor *qov, const char *name, QObject *value) {
    QStackEntry *e = QSLIST_FIRST(&qov->stack);
...
static bool qobject_output_start_struct(Visitor *v, const char *name, void **obj, size_t unused, Error **errp) {
...
static bool qobject_output_start_list(Visitor *v, const char *name, GenericList **listp, size_t size, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
...
static GenericList *qobject_output_next_list(Visitor *v, GenericList *tail, size_t size) {
    return tail->next;
}
...
static void qobject_output_end_list(Visitor *v, void **obj) {
    QObjectOutputVisitor *qov = to_qov(v);
    QObject *value = qobject_output_pop(qov, obj);
    if (qobject_type(value) != QTYPE_QLIST) {
        perror("Invalid type");
    }
}

static bool qobject_output_type_int64(Visitor *v, const char *name, int64_t *obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    qobject_output_add(qov, name, qnum_from_int(*obj));
    return true;
}

static bool qobject_output_type_uint64(Visitor *v, const char *name, uint64_t *obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    qobject_output_add(qov, name, qnum_from_uint(*obj));
    return true;
}

static bool qobject_output_type_bool(Visitor *v, const char *name, bool *obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    qobject_output_add(qov, name, qbool_from_bool(*obj));
    return true;
}

static bool qobject_output_type_str(Visitor *v, const char *name, char **obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    if (*obj) {
...
static bool qobject_output_type_number(Visitor *v, const char *name, double *obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    qobject_output_add(qov, name, qnum_from_double(*obj));
    return true;
}

static bool qobject_output_type_any(Visitor *v, const char *name, QObject **obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
...
static bool qobject_output_type_null(Visitor *v, const char *name, QNull **obj, Error **errp) {
    QObjectOutputVisitor *qov = to_qov(v);
    qobject_output_add(qov, name, qnull());
    return true;
}

static bool qobject_output_policy_skip(Visitor *v, const char *name, unsigned special_features) {
    CompatPolicy *pol = &v->compat_policy;

    return ((special_features & 1u << QAPI_DEPRECATED) && pol->deprecated_output == COMPAT_POLICY_OUTPUT_HIDE) ||
           ((special_features & 1u << QAPI_UNSTABLE) && pol->unstable_output == COMPAT_POLICY_OUTPUT_HIDE);
}
