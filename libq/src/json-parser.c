...
static QString *qstring_from_escaped_str(JSONParserContext *ctxt, QObject *token) {
    ...
    str = qstring_new();
    while (*ptr && ((double_quote && *ptr != '"') || (!double_quote && *ptr != '\''))) {
        if (*ptr == '\\') {
            ptr++;
                ...
                case 'u': {
                    ...
                    for (i = 0; i < 4; i++) {
                        if (isxdigit(*ptr)) {
                            ...
                            parse_error(ctxt, token, "invalid hex escape sequence in string");
                            goto out;
                        }
                        ptr++;
                    }
...
static QObject *parse_escape(JSONParserContext *ctxt, QList **tokens, va_list *ap) {
    ...
    } else if (token_is_escape(token, "%lld") || token_is_escape(token, "%I64d")) {
