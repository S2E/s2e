...
static inline uint8_t next_state(JSONLexer *lexer, char ch, bool flush, bool *char_consumed) {
    uint8_t next;
...
void json_lexer_init(JSONLexer *lexer, bool enable_interpolation) {
    lexer->start_state = lexer->state = enable_interpolation ? IN_START_INTERP : IN_START;
...
static void json_lexer_feed_char(JSONLexer *lexer, char ch, bool flush) {
    ...
    while (flush ? lexer->state != lexer->start_state : !char_consumed) {
        ...
        switch (new_state) {
            ...
            case IN_START:
                ...
                break;
            case JSON_ERROR:
                json_message_process_token(lexer, lexer->token, JSON_ERROR, lexer->x, lexer->y);
    ...
    if (lexer->token->len > MAX_TOKEN_SIZE) {
        json_message_process_token(lexer, lexer->token, lexer->state, lexer->x, lexer->y);
        g_string_truncate(lexer->token, 0);
...
void json_lexer_flush(JSONLexer *lexer) {
    ...
    json_message_process_token(lexer, lexer->token, JSON_END_OF_INPUT, lexer->x, lexer->y);
}
