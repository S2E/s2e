 ...
 * Error handler
 */
static void G_GNUC_PRINTF(3, 4) parse_error(JSONParserContext *ctxt, JSONToken *token, const char *msg, ...) {
    va_list ap;
    char message[1024];
                ...
                switch (*ptr++) {
                    ...
                    case 'u':
                        ...
                        if (mod_utf8_encode(utf8_buf, sizeof(utf8_buf), cp) < 0) {
                            parse_error(ctxt, token, "%.*s is not a valid Unicode character", (int) (ptr - beg), beg);
                            goto out;
                        }
...
static QObject *parse_literal(JSONParserContext *ctxt) {
    ...
    switch (token->type) {
        ...
        case JSON_INTEGER: {
             ...
             * QNUM_U64, else as QNUM_DOUBLE.  Note that libq_strtoi64()
             * and libq_strtou64() fail with ERANGE when it's not
             * possible.
             *
            ...
            ret = libq_strtoi64(token->str, NULL, 10, &value);
            if (!ret) {
            ...
            if (token->str[0] != '-') {
                ret = libq_strtou64(token->str, NULL, 10, &uvalue);
                if (!ret) {
