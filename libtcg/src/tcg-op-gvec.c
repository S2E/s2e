 ...
 */
static void check_overlap_2(TCGv_ptr dbase, uint32_t d, TCGv_ptr abase, uint32_t a, uint32_t s) {
    tcg_debug_assert(dbase != abase || d == a || d + s <= a || a + s <= d);
}

/* Verify vector overlap rules for three operands.  */
static void check_overlap_3(TCGv_ptr dbase, uint32_t d, TCGv_ptr abase, uint32_t a, TCGv_ptr bbase, uint32_t b,
                            uint32_t s) {
...
static void check_overlap_4(TCGv_ptr dbase, uint32_t d, TCGv_ptr abase, uint32_t a, TCGv_ptr bbase, uint32_t b,
...
uint32_t simd_desc(uint32_t oprsz, uint32_t maxsz, int32_t data) {
     ...
     */
    tcg_debug_assert(data == sextract32(data, 0, SIMD_DATA_BITS) || data == extract32(data, 0, SIMD_DATA_BITS));
...
static void expand_2_ool(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz, uint32_t maxsz,
...
static void expand_3_ool(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase, uint32_t bofs,
                         uint32_t oprsz, uint32_t maxsz, int32_t data, gen_helper_gvec_3 *fn) {
    TCGv_ptr a0, a1, a2;
    TCGv_i32 desc = tcg_constant_i32(simd_desc(oprsz, maxsz, data));
...
void tcg_gen_gvec_3_ool(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz, int32_t data,
    ...
    expand_3_ool(tcg_env, dofs, tcg_env, aofs, tcg_env, bofs, oprsz, maxsz, data, fn);
}
...
/* Generate a call to a gvec-style helper with five vector operands
   and an extra pointer operand.  */
void tcg_gen_gvec_5_ptr(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t eofs, TCGv_ptr ptr,
                        uint32_t oprsz, uint32_t maxsz, int32_t data, gen_helper_gvec_5_ptr *fn) {
    TCGv_ptr a0, a1, a2, a3, a4;
    TCGv_i32 desc = tcg_constant_i32(simd_desc(oprsz, maxsz, data));
 ...
 */
static TCGType choose_vector_type(const TCGOpcode *list, unsigned vece, uint32_t size, bool prefer_i64) {
    /*
     * Recall that ARM SVE allows vector sizes that are not a
     * power of 2, but always a multiple of 16.  The intent is
     * that e.g. size == 80 would be expanded with 2x32 + 1x16.
     ...
     * In addition, expand_clr needs to handle a multiple of 8.
     */
    if (TCG_TARGET_HAS_v256 && check_size_impl(size, 32) && tcg_can_emit_vecop_list(list, TCG_TYPE_V256, vece) &&
        (!(size & 16) || (TCG_TARGET_HAS_v128 && tcg_can_emit_vecop_list(list, TCG_TYPE_V128, vece))) &&
        (!(size & 8) || (TCG_TARGET_HAS_v64 && tcg_can_emit_vecop_list(list, TCG_TYPE_V64, vece)))) {
        return TCG_TYPE_V256;
    }
    if (TCG_TARGET_HAS_v128 && check_size_impl(size, 16) && tcg_can_emit_vecop_list(list, TCG_TYPE_V128, vece) &&
        (!(size & 8) || (TCG_TARGET_HAS_v64 && tcg_can_emit_vecop_list(list, TCG_TYPE_V64, vece)))) {
...
static void do_dup_store(TCGType type, TCGv_ptr dbase, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, TCGv_vec t_vec) {
    uint32_t i = 0;
 ...
 */
static void do_dup(unsigned vece, TCGv_ptr dbase, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, TCGv_i32 in_32,
                   TCGv_i64 in_64, uint64_t in_c) {
    TCGType type;
    TCGv_i64 t_64;
    ...
    type = choose_vector_type(NULL, vece, oprsz,
                              (TCG_TARGET_REG_BITS == 64 && in_32 == NULL && (in_64 == NULL || vece == MO_64)));
    if (type != 0) {
    ...
    if (check_size_impl(oprsz, TCG_TARGET_REG_BITS / 8)) {
        ...
        if (in_32) {
            /* We are given a 32-bit variable input.  For a 64-bit host,
               ...
               be simple enough.  */
            if (TCG_TARGET_REG_BITS == 64 && (vece != MO_32 || !check_size_impl(oprsz, 4))) {
            ...
            if (vece == MO_64 ||
                (TCG_TARGET_REG_BITS == 64 && (in_c == 0 || in_c == -1 || !check_size_impl(oprsz, 4)))) {
    ...
    if (vece == MO_64) {
        ...
        static dup_fn *const fns[3] = {gen_helper_gvec_dup8, gen_helper_gvec_dup16, gen_helper_gvec_dup32};
...
static void expand_2_i32(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz, bool load_dest,
...
static void expand_2i_i32(uint32_t dofs, uint32_t aofs, uint32_t oprsz, int32_t c, bool load_dest,
...
static void expand_2s_i32(uint32_t dofs, uint32_t aofs, uint32_t oprsz, TCGv_i32 c, bool scalar_first,
...
static void expand_3_i32(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase, uint32_t bofs,
                         uint32_t oprsz, bool load_dest, void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32)) {
    TCGv_i32 t0 = tcg_temp_new_i32();
    TCGv_i32 t1 = tcg_temp_new_i32();
...
static void expand_3i_i32(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, int32_t c, bool load_dest,
                          bool write_aofs, void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32, int32_t)) {
    TCGv_i32 t0 = tcg_temp_new_i32();
    TCGv_i32 t1 = tcg_temp_new_i32();
    TCGv_i32 t2 = tcg_temp_new_i32();
    uint32_t i;
...
/* Expand OPSZ bytes worth of three-operand operations using i32 elements.  */
static void expand_4_i32(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, bool write_aofs,
...
static void expand_4i_i32(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, int32_t c,
                          void (*fni)(TCGv_i32, TCGv_i32, TCGv_i32, TCGv_i32, int32_t)) {
    TCGv_i32 t0 = tcg_temp_new_i32();
    TCGv_i32 t1 = tcg_temp_new_i32();
    TCGv_i32 t2 = tcg_temp_new_i32();
    TCGv_i32 t3 = tcg_temp_new_i32();
...
static void expand_2_i64(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz, bool load_dest,
...
static void expand_2i_i64(uint32_t dofs, uint32_t aofs, uint32_t oprsz, int64_t c, bool load_dest,
...
static void expand_2s_i64(uint32_t dofs, uint32_t aofs, uint32_t oprsz, TCGv_i64 c, bool scalar_first,
...
static void expand_3_i64(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase, uint32_t bofs,
                         uint32_t oprsz, bool load_dest, void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64)) {
    TCGv_i64 t0 = tcg_temp_new_i64();
    TCGv_i64 t1 = tcg_temp_new_i64();
...
static void expand_3i_i64(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, int64_t c, bool load_dest,
                          bool write_aofs, void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64, int64_t)) {
    TCGv_i64 t0 = tcg_temp_new_i64();
    TCGv_i64 t1 = tcg_temp_new_i64();
    TCGv_i64 t2 = tcg_temp_new_i64();
    uint32_t i;
...
/* Expand OPSZ bytes worth of three-operand operations using i64 elements.  */
static void expand_4_i64(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, bool write_aofs,
...
static void expand_4i_i64(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, int64_t c,
                          void (*fni)(TCGv_i64, TCGv_i64, TCGv_i64, TCGv_i64, int64_t)) {
    TCGv_i64 t0 = tcg_temp_new_i64();
    TCGv_i64 t1 = tcg_temp_new_i64();
    TCGv_i64 t2 = tcg_temp_new_i64();
    TCGv_i64 t3 = tcg_temp_new_i64();
...
static void expand_2_vec(unsigned vece, TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz,
                         uint32_t tysz, TCGType type, bool load_dest, void (*fni)(unsigned, TCGv_vec, TCGv_vec)) {
...
/* Expand OPSZ bytes worth of two-vector operands and an immediate operand
   using host vectors.  */
static void expand_2i_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t tysz, TCGType type,
                          int64_t c, bool load_dest, void (*fni)(unsigned, TCGv_vec, TCGv_vec, int64_t)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
...
static void expand_2s_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t tysz, TCGType type,
                          TCGv_vec c, bool scalar_first, void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
...
static void expand_3_vec(unsigned vece, TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase,
                         uint32_t bofs, uint32_t oprsz, uint32_t tysz, TCGType type, bool load_dest,
 ...
 * using host vectors.
 */
static void expand_3i_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t tysz,
                          TCGType type, int64_t c, bool load_dest, bool write_aofs,
                          void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec, int64_t)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
        TCGv_vec t2 = tcg_temp_new_vec(type);
...
static void expand_4_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz,
                         uint32_t tysz, TCGType type, bool write_aofs,
                         void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec, TCGv_vec)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
        TCGv_vec t2 = tcg_temp_new_vec(type);
 ...
 * using host vectors.
 */
static void expand_4i_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz,
                          uint32_t tysz, TCGType type, int64_t c,
                          void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_vec, TCGv_vec, int64_t)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
        TCGv_vec t2 = tcg_temp_new_vec(type);
...
void tcg_gen_gvec_2_var(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz, uint32_t maxsz,
                        const GVecGen2 *g) {
    ...
    switch (type) {
        case TCG_TYPE_V256:
            ...
            expand_2_vec(g->vece, dbase, dofs, abase, aofs, some, 32, TCG_TYPE_V256, g->load_dest, g->fniv);
            if (some == oprsz) {
                break;
            }
            dofs += some;
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_2_vec(g->vece, dbase, dofs, abase, aofs, oprsz, 16, TCG_TYPE_V128, g->load_dest, g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_2_vec(g->vece, dbase, dofs, abase, aofs, oprsz, 8, TCG_TYPE_V64, g->load_dest, g->fniv);
            break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) {
                expand_2_i64(dbase, dofs, abase, aofs, oprsz, g->load_dest, g->fni8);
            } else if (g->fni4 && check_size_impl(oprsz, 4)) {
                expand_2_i32(dbase, dofs, abase, aofs, oprsz, g->load_dest, g->fni4);
            } else {
                assert(g->fno != NULL);
                expand_2_ool(dbase, dofs, abase, aofs, oprsz, maxsz, g->data, g->fno);
                oprsz = maxsz;
            }
            break;
...
void tcg_gen_gvec_2(uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz, const GVecGen2 *g) {
    tcg_gen_gvec_2_var(tcg_env, dofs, tcg_env, aofs, oprsz, maxsz, g);
}

/* Expand a vector operation with two vectors and an immediate.  */
void tcg_gen_gvec_2i(uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz, int64_t c, const GVecGen2i *g) {
    const TCGOpcode *this_list = g->opt_opc ?: vecop_list_empty;
    const TCGOpcode *hold_list = tcg_swap_vecop_list(this_list);
    TCGType type;
    uint32_t some;
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             */
            some = QEMU_ALIGN_DOWN(oprsz, 32);
            expand_2i_vec(g->vece, dofs, aofs, some, 32, TCG_TYPE_V256, c, g->load_dest, g->fniv);
            if (some == oprsz) {
                break;
            }
            dofs += some;
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_2i_vec(g->vece, dofs, aofs, oprsz, 16, TCG_TYPE_V128, c, g->load_dest, g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_2i_vec(g->vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, c, g->load_dest, g->fniv);
            break;
...
void tcg_gen_gvec_2s(uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz, TCGv_i64 c, const GVecGen2s *g) {
    TCGType type;
    ...
    if (type != 0) {
        ...
        switch (type) {
            case TCG_TYPE_V256:
                 ...
                 */
                some = QEMU_ALIGN_DOWN(oprsz, 32);
                expand_2s_vec(g->vece, dofs, aofs, some, 32, TCG_TYPE_V256, t_vec, g->scalar_first, g->fniv);
                if (some == oprsz) {
                    break;
                }
                dofs += some;
            ...
            case TCG_TYPE_V64:
                expand_2s_vec(g->vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, t_vec, g->scalar_first, g->fniv);
                break;
...
void tcg_gen_gvec_3_var(TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase, uint32_t bofs,
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             */
            some = QEMU_ALIGN_DOWN(oprsz, 32);
            expand_3_vec(g->vece, dbase, dofs, abase, aofs, bbase, bofs, some, 32, TCG_TYPE_V256, g->load_dest,
                         g->fniv);
            if (some == oprsz) {
                break;
            }
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_3_vec(g->vece, dbase, dofs, abase, aofs, bbase, bofs, oprsz, 16, TCG_TYPE_V128, g->load_dest,
                         g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_3_vec(g->vece, dbase, dofs, abase, aofs, bbase, bofs, oprsz, 8, TCG_TYPE_V64, g->load_dest, g->fniv);
            break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) {
                expand_3_i64(dbase, dofs, abase, aofs, bbase, bofs, oprsz, g->load_dest, g->fni8);
            } else if (g->fni4 && check_size_impl(oprsz, 4)) {
                expand_3_i32(dbase, dofs, abase, aofs, bbase, bofs, oprsz, g->load_dest, g->fni4);
            } else {
                assert(g->fno != NULL);
                expand_3_ool(dbase, dofs, abase, aofs, bbase, bofs, oprsz, maxsz, g->data, g->fno);
                oprsz = maxsz;
            }
            break;
...
void tcg_gen_gvec_3(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz, const GVecGen3 *g) {
    tcg_gen_gvec_3_var(tcg_env, dofs, tcg_env, aofs, tcg_env, bofs, oprsz, maxsz, g);
}

/* Expand a vector operation with three vectors and an immediate.  */
void tcg_gen_gvec_3i(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz, int64_t c,
                     const GVecGen3i *g) {
    const TCGOpcode *this_list = g->opt_opc ?: vecop_list_empty;
    const TCGOpcode *hold_list = tcg_swap_vecop_list(this_list);
    TCGType type;
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             */
            some = QEMU_ALIGN_DOWN(oprsz, 32);
            expand_3i_vec(g->vece, dofs, aofs, bofs, some, 32, TCG_TYPE_V256, c, g->load_dest, g->write_aofs, g->fniv);
            if (some == oprsz) {
                break;
            }
            dofs += some;
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_3i_vec(g->vece, dofs, aofs, bofs, oprsz, 16, TCG_TYPE_V128, c, g->load_dest, g->write_aofs, g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_3i_vec(g->vece, dofs, aofs, bofs, oprsz, 8, TCG_TYPE_V64, c, g->load_dest, g->write_aofs, g->fniv);
            break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) {
                expand_3i_i64(dofs, aofs, bofs, oprsz, c, g->load_dest, g->write_aofs, g->fni8);
            } else if (g->fni4 && check_size_impl(oprsz, 4)) {
                expand_3i_i32(dofs, aofs, bofs, oprsz, c, g->load_dest, g->write_aofs, g->fni4);
            } else {
                assert(g->fno != NULL);
...
void tcg_gen_gvec_4(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, uint32_t maxsz,
    ...
    check_overlap_4(tcg_env, dofs, tcg_env, aofs, tcg_env, bofs, tcg_env, cofs, maxsz);
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             */
            some = QEMU_ALIGN_DOWN(oprsz, 32);
            expand_4_vec(g->vece, dofs, aofs, bofs, cofs, some, 32, TCG_TYPE_V256, g->write_aofs, g->fniv);
            if (some == oprsz) {
                break;
            }
            dofs += some;
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_4_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 16, TCG_TYPE_V128, g->write_aofs, g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_4_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 8, TCG_TYPE_V64, g->write_aofs, g->fniv);
            break;

        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) {
                expand_4_i64(dofs, aofs, bofs, cofs, oprsz, g->write_aofs, g->fni8);
            } else if (g->fni4 && check_size_impl(oprsz, 4)) {
                expand_4_i32(dofs, aofs, bofs, cofs, oprsz, g->write_aofs, g->fni4);
            } else {
                assert(g->fno != NULL);
...
/* Expand a vector four-operand operation.  */
void tcg_gen_gvec_4i(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz, uint32_t maxsz,
                     int64_t c, const GVecGen4i *g) {
    const TCGOpcode *this_list = g->opt_opc ?: vecop_list_empty;
    const TCGOpcode *hold_list = tcg_swap_vecop_list(this_list);
    TCGType type;
    uint32_t some;
    ...
    switch (type) {
        case TCG_TYPE_V256:
             ...
             */
            some = QEMU_ALIGN_DOWN(oprsz, 32);
            expand_4i_vec(g->vece, dofs, aofs, bofs, cofs, some, 32, TCG_TYPE_V256, c, g->fniv);
            if (some == oprsz) {
                break;
            }
            dofs += some;
            ...
            /* fallthru */
        case TCG_TYPE_V128:
            expand_4i_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 16, TCG_TYPE_V128, c, g->fniv);
            break;
        case TCG_TYPE_V64:
            expand_4i_vec(g->vece, dofs, aofs, bofs, cofs, oprsz, 8, TCG_TYPE_V64, c, g->fniv);
            break;
        ...
        case 0:
            if (g->fni8 && check_size_impl(oprsz, 8)) {
                ...
                assert(g->fno != NULL);
                tcg_gen_gvec_4_ool(dofs, aofs, bofs, cofs, oprsz, maxsz, c, g->fno);
                oprsz = maxsz;
            }
            break;
...
void tcg_gen_gvec_mov_var(unsigned vece, TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, uint32_t oprsz,
                          uint32_t maxsz) {
...
void tcg_gen_gvec_dup_i32(unsigned vece, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, TCGv_i32 in) {
    check_size_align(oprsz, maxsz, dofs);
    tcg_debug_assert(vece <= MO_32);
    do_dup(vece, tcg_env, dofs, oprsz, maxsz, in, NULL, 0);
}

void tcg_gen_gvec_dup_i64(unsigned vece, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, TCGv_i64 in) {
    check_size_align(oprsz, maxsz, dofs);
    tcg_debug_assert(vece <= MO_64);
    do_dup(vece, tcg_env, dofs, oprsz, maxsz, NULL, in, 0);
}

void tcg_gen_gvec_dup_mem(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz) {
    check_size_align(oprsz, maxsz, dofs);
    if (vece <= MO_64) {
...
void tcg_gen_gvec_dup_imm_var(unsigned vece, TCGv_ptr dbase, uint32_t dofs, uint32_t oprsz, uint32_t maxsz,
                              uint64_t x) {
    check_size_align(oprsz, maxsz, dofs);
    do_dup(vece, dbase, dofs, oprsz, maxsz, NULL, NULL, x);
}

void tcg_gen_gvec_dup_imm(unsigned vece, uint32_t dofs, uint32_t oprsz, uint32_t maxsz, uint64_t x) {
...
void tcg_gen_gvec_add_var(unsigned vece, TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase,
    ...
    tcg_debug_assert(vece <= MO_64);
    tcg_gen_gvec_3_var(dbase, dofs, abase, aofs, bbase, bofs, oprsz, maxsz, &g[vece]);
}

void tcg_gen_gvec_add(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    tcg_gen_gvec_add_var(vece, tcg_env, dofs, tcg_env, aofs, tcg_env, bofs, oprsz, maxsz);
}
...
void tcg_gen_gvec_sub_var(unsigned vece, TCGv_ptr dbase, uint32_t dofs, TCGv_ptr abase, uint32_t aofs, TCGv_ptr bbase,
                          uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen3 g[4] = {
        {.fni8 = tcg_gen_vec_sub8_i64,
         .fniv = tcg_gen_sub_vec,
...
void tcg_gen_gvec_sub(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    tcg_gen_gvec_sub_var(vece, tcg_env, dofs, tcg_env, aofs, tcg_env, bofs, oprsz, maxsz);
}
...
void tcg_gen_gvec_mul(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_mul_vec, .fno = gen_helper_gvec_mul8, .opt_opc = vecop_list_mul, .vece = MO_8},
        {.fniv = tcg_gen_mul_vec, .fno = gen_helper_gvec_mul16, .opt_opc = vecop_list_mul, .vece = MO_16},
...
void tcg_gen_gvec_muls(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i64 c, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen2s g[4] = {
        {.fniv = tcg_gen_mul_vec, .fno = gen_helper_gvec_muls8, .opt_opc = vecop_list_mul, .vece = MO_8},
        {.fniv = tcg_gen_mul_vec, .fno = gen_helper_gvec_muls16, .opt_opc = vecop_list_mul, .vece = MO_16},
        {.fni4 = tcg_gen_mul_i32,
         .fniv = tcg_gen_mul_vec,
...
void tcg_gen_gvec_ssadd(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_ssadd_vec, .fno = gen_helper_gvec_ssadd8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_ssadd_vec, .fno = gen_helper_gvec_ssadd16, .opt_opc = vecop_list, .vece = MO_16},
        {.fniv = tcg_gen_ssadd_vec, .fno = gen_helper_gvec_ssadd32, .opt_opc = vecop_list, .vece = MO_32},
        {.fniv = tcg_gen_ssadd_vec, .fno = gen_helper_gvec_ssadd64, .opt_opc = vecop_list, .vece = MO_64},
    };
    tcg_debug_assert(vece <= MO_64);
    tcg_gen_gvec_3(dofs, aofs, bofs, oprsz, maxsz, &g[vece]);
}
...
void tcg_gen_gvec_sssub(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_sssub_vec, .fno = gen_helper_gvec_sssub8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_sssub_vec, .fno = gen_helper_gvec_sssub16, .opt_opc = vecop_list, .vece = MO_16},
        {.fniv = tcg_gen_sssub_vec, .fno = gen_helper_gvec_sssub32, .opt_opc = vecop_list, .vece = MO_32},
        {.fniv = tcg_gen_sssub_vec, .fno = gen_helper_gvec_sssub64, .opt_opc = vecop_list, .vece = MO_64},
    };
    tcg_debug_assert(vece <= MO_64);
    tcg_gen_gvec_3(dofs, aofs, bofs, oprsz, maxsz, &g[vece]);
}
...
void tcg_gen_gvec_usadd(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_usadd_vec, .fno = gen_helper_gvec_usadd8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_usadd_vec, .fno = gen_helper_gvec_usadd16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_ussub(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_ussub_vec, .fno = gen_helper_gvec_ussub8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_ussub_vec, .fno = gen_helper_gvec_ussub16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_smin(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_smin_vec, .fno = gen_helper_gvec_smin8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_smin_vec, .fno = gen_helper_gvec_smin16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_umin(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_umin_vec, .fno = gen_helper_gvec_umin8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_umin_vec, .fno = gen_helper_gvec_umin16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_smax(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_smax_vec, .fno = gen_helper_gvec_smax8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_smax_vec, .fno = gen_helper_gvec_smax16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_umax(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    ...
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_umax_vec, .fno = gen_helper_gvec_umax8, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_umax_vec, .fno = gen_helper_gvec_umax16, .opt_opc = vecop_list, .vece = MO_16},
...
void tcg_gen_gvec_abs(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t maxsz) {
...
static const GVecGen2s gop_ands = {.fni8 = tcg_gen_and_i64,
                                   .fniv = tcg_gen_and_vec,
...
void tcg_gen_gvec_andcs(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i64 c, uint32_t oprsz, uint32_t maxsz) {
    static GVecGen2s g = {.fni8 = tcg_gen_andc_i64,
                          .fniv = tcg_gen_andc_vec,
...
static const GVecGen2s gop_xors = {.fni8 = tcg_gen_xor_i64,
                                   .fniv = tcg_gen_xor_vec,
...
static const GVecGen2s gop_ors = {.fni8 = tcg_gen_or_i64,
                                  .fniv = tcg_gen_or_vec,
...
void tcg_gen_gvec_rotli(unsigned vece, uint32_t dofs, uint32_t aofs, int64_t shift, uint32_t oprsz, uint32_t maxsz) {
...
void tcg_gen_gvec_rotri(unsigned vece, uint32_t dofs, uint32_t aofs, int64_t shift, uint32_t oprsz, uint32_t maxsz) {
    tcg_debug_assert(vece <= MO_64);
    tcg_debug_assert(shift >= 0 && shift < (8 << vece));
    tcg_gen_gvec_rotli(vece, dofs, aofs, -shift & ((8 << vece) - 1), oprsz, maxsz);
}
...
static void expand_2sh_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t tysz, TCGType type,
                           TCGv_i32 shift, void (*fni)(unsigned, TCGv_vec, TCGv_vec, TCGv_i32)) {
    for (uint32_t i = 0; i < oprsz; i += tysz) {
        TCGv_vec t0 = tcg_temp_new_vec(type);
        TCGv_vec t1 = tcg_temp_new_vec(type);
...
static void do_gvec_shifts(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz,
                           const GVecGen2sh *g) {
    TCGType type;
    uint32_t some;
    ...
    if (type) {
        ...
        switch (type) {
            case TCG_TYPE_V256:
                some = QEMU_ALIGN_DOWN(oprsz, 32);
                expand_2sh_vec(vece, dofs, aofs, some, 32, TCG_TYPE_V256, shift, g->fniv_s);
                if (some == oprsz) {
                    break;
                }
                dofs += some;
                ...
                /* fallthru */
            case TCG_TYPE_V128:
                expand_2sh_vec(vece, dofs, aofs, oprsz, 16, TCG_TYPE_V128, shift, g->fniv_s);
                break;
            case TCG_TYPE_V64:
                expand_2sh_vec(vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, shift, g->fniv_s);
                break;
            default:
                g_assert_not_reached();
        }
    ...
    if (type) {
        ...
        switch (type) {
            case TCG_TYPE_V256:
                some = QEMU_ALIGN_DOWN(oprsz, 32);
                expand_2s_vec(vece, dofs, aofs, some, 32, TCG_TYPE_V256, v_shift, false, g->fniv_v);
                if (some == oprsz) {
                    break;
                }
                dofs += some;
                ...
                /* fallthru */
            case TCG_TYPE_V128:
                expand_2s_vec(vece, dofs, aofs, oprsz, 16, TCG_TYPE_V128, v_shift, false, g->fniv_v);
                break;
            case TCG_TYPE_V64:
                expand_2s_vec(vece, dofs, aofs, oprsz, 8, TCG_TYPE_V64, v_shift, false, g->fniv_v);
                break;
            default:
                g_assert_not_reached();
        }
...
void tcg_gen_gvec_shls(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz) {
...
void tcg_gen_gvec_shrs(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen2sh g = {
...
void tcg_gen_gvec_sars(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen2sh g = {
...
void tcg_gen_gvec_rotls(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz) {
    static const GVecGen2sh g = {
...
void tcg_gen_gvec_rotrs(unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i32 shift, uint32_t oprsz, uint32_t maxsz) {
 ...
 */
static void tcg_gen_shlv_mod_vec(unsigned vece, TCGv_vec d, TCGv_vec a, TCGv_vec b) {
...
void tcg_gen_gvec_shlv(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const TCGOpcode vecop_list[] = {INDEX_op_shlv_vec, 0};
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_shlv_mod_vec, .fno = gen_helper_gvec_shl8v, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_shlv_mod_vec, .fno = gen_helper_gvec_shl16v, .opt_opc = vecop_list, .vece = MO_16},
...
static void tcg_gen_shrv_mod_vec(unsigned vece, TCGv_vec d, TCGv_vec a, TCGv_vec b) {
    TCGv_vec t = tcg_temp_new_vec_matching(d);
    TCGv_vec m = tcg_constant_vec_matching(d, vece, (8 << vece) - 1);
...
void tcg_gen_gvec_shrv(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const TCGOpcode vecop_list[] = {INDEX_op_shrv_vec, 0};
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_shrv_mod_vec, .fno = gen_helper_gvec_shr8v, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_shrv_mod_vec, .fno = gen_helper_gvec_shr16v, .opt_opc = vecop_list, .vece = MO_16},
...
static void tcg_gen_sarv_mod_vec(unsigned vece, TCGv_vec d, TCGv_vec a, TCGv_vec b) {
    TCGv_vec t = tcg_temp_new_vec_matching(d);
    TCGv_vec m = tcg_constant_vec_matching(d, vece, (8 << vece) - 1);
...
void tcg_gen_gvec_sarv(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const TCGOpcode vecop_list[] = {INDEX_op_sarv_vec, 0};
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_sarv_mod_vec, .fno = gen_helper_gvec_sar8v, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_sarv_mod_vec, .fno = gen_helper_gvec_sar16v, .opt_opc = vecop_list, .vece = MO_16},
...
static void tcg_gen_rotlv_mod_vec(unsigned vece, TCGv_vec d, TCGv_vec a, TCGv_vec b) {
    TCGv_vec t = tcg_temp_new_vec_matching(d);
    TCGv_vec m = tcg_constant_vec_matching(d, vece, (8 << vece) - 1);
...
void tcg_gen_gvec_rotlv(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const TCGOpcode vecop_list[] = {INDEX_op_rotlv_vec, 0};
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_rotlv_mod_vec, .fno = gen_helper_gvec_rotl8v, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_rotlv_mod_vec, .fno = gen_helper_gvec_rotl16v, .opt_opc = vecop_list, .vece = MO_16},
...
static void tcg_gen_rotrv_mod_vec(unsigned vece, TCGv_vec d, TCGv_vec a, TCGv_vec b) {
    TCGv_vec t = tcg_temp_new_vec_matching(d);
    TCGv_vec m = tcg_constant_vec_matching(d, vece, (8 << vece) - 1);
...
void tcg_gen_gvec_rotrv(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t maxsz) {
    static const TCGOpcode vecop_list[] = {INDEX_op_rotrv_vec, 0};
    static const GVecGen3 g[4] = {
        {.fniv = tcg_gen_rotrv_mod_vec, .fno = gen_helper_gvec_rotr8v, .opt_opc = vecop_list, .vece = MO_8},
        {.fniv = tcg_gen_rotrv_mod_vec, .fno = gen_helper_gvec_rotr16v, .opt_opc = vecop_list, .vece = MO_16},
...
/* Expand OPSZ bytes worth of three-operand operations using i32 elements.  */
static void expand_cmp_i32(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, TCGCond cond) {
...
static void expand_cmp_i64(uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, TCGCond cond) {
...
static void expand_cmp_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz, uint32_t tysz,
...
void tcg_gen_gvec_cmp(TCGCond cond, unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t oprsz,
    ...
    static gen_helper_gvec_3 *const eq_fn[4] = {gen_helper_gvec_eq8, gen_helper_gvec_eq16, gen_helper_gvec_eq32,
                                                gen_helper_gvec_eq64};
    static gen_helper_gvec_3 *const ne_fn[4] = {gen_helper_gvec_ne8, gen_helper_gvec_ne16, gen_helper_gvec_ne32,
                                                gen_helper_gvec_ne64};
    static gen_helper_gvec_3 *const lt_fn[4] = {gen_helper_gvec_lt8, gen_helper_gvec_lt16, gen_helper_gvec_lt32,
                                                gen_helper_gvec_lt64};
    static gen_helper_gvec_3 *const le_fn[4] = {gen_helper_gvec_le8, gen_helper_gvec_le16, gen_helper_gvec_le32,
                                                gen_helper_gvec_le64};
    static gen_helper_gvec_3 *const ltu_fn[4] = {gen_helper_gvec_ltu8, gen_helper_gvec_ltu16, gen_helper_gvec_ltu32,
    ...
    if (cond == TCG_COND_NEVER || cond == TCG_COND_ALWAYS) {
        do_dup(MO_8, tcg_env, dofs, oprsz, maxsz, NULL, NULL, -(cond == TCG_COND_ALWAYS));
        return;
    }
    ...
    type = choose_vector_type(cmp_list, vece, oprsz, TCG_TARGET_REG_BITS == 64 && vece == MO_64);
    switch (type) {
        case TCG_TYPE_V256:
            /* Recall that ARM SVE allows vector sizes that are not a
             * power of 2, but always a multiple of 16.  The intent is
...
static void expand_cmps_vec(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t oprsz, uint32_t tysz, TCGType type,
...
void tcg_gen_gvec_cmps(TCGCond cond, unsigned vece, uint32_t dofs, uint32_t aofs, TCGv_i64 c, uint32_t oprsz,
                       uint32_t maxsz) {
    static const TCGOpcode cmp_list[] = {INDEX_op_cmp_vec, 0};
    static gen_helper_gvec_2i *const eq_fn[4] = {gen_helper_gvec_eqs8, gen_helper_gvec_eqs16, gen_helper_gvec_eqs32,
                                                 gen_helper_gvec_eqs64};
    static gen_helper_gvec_2i *const lt_fn[4] = {gen_helper_gvec_lts8, gen_helper_gvec_lts16, gen_helper_gvec_lts32,
                                                 gen_helper_gvec_lts64};
    static gen_helper_gvec_2i *const le_fn[4] = {gen_helper_gvec_les8, gen_helper_gvec_les16, gen_helper_gvec_les32,
                                                 gen_helper_gvec_les64};
    static gen_helper_gvec_2i *const ltu_fn[4] = {gen_helper_gvec_ltus8, gen_helper_gvec_ltus16, gen_helper_gvec_ltus32,
                                                  gen_helper_gvec_ltus64};
    static gen_helper_gvec_2i *const leu_fn[4] = {gen_helper_gvec_leus8, gen_helper_gvec_leus16, gen_helper_gvec_leus32,
                                                  gen_helper_gvec_leus64};
    static gen_helper_gvec_2i *const *const fns[16] = {
        [TCG_COND_EQ] = eq_fn,   [TCG_COND_LT] = lt_fn,   [TCG_COND_LE] = le_fn,
        [TCG_COND_LTU] = ltu_fn, [TCG_COND_LEU] = leu_fn,
    };
    ...
    if (type != 0) {
        ...
        switch (type) {
            case TCG_TYPE_V256:
                some = QEMU_ALIGN_DOWN(oprsz, 32);
                expand_cmps_vec(vece, dofs, aofs, some, 32, TCG_TYPE_V256, cond, t_vec);
                aofs += some;
                dofs += some;
                oprsz -= some;
                maxsz -= some;
            ...
            case TCG_TYPE_V128:
                ...
                expand_cmps_vec(vece, dofs, aofs, some, 16, TCG_TYPE_V128, cond, t_vec);
                break;

            case TCG_TYPE_V64:
                some = QEMU_ALIGN_DOWN(oprsz, 8);
                expand_cmps_vec(vece, dofs, aofs, some, 8, TCG_TYPE_V64, cond, t_vec);
                break;
...
void tcg_gen_gvec_cmpi(TCGCond cond, unsigned vece, uint32_t dofs, uint32_t aofs, int64_t c, uint32_t oprsz,
                       uint32_t maxsz) {
    TCGv_i64 tmp = tcg_constant_i64(c);
...
void tcg_gen_gvec_bitsel(unsigned vece, uint32_t dofs, uint32_t aofs, uint32_t bofs, uint32_t cofs, uint32_t oprsz,
                         uint32_t maxsz) {
