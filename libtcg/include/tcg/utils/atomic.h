...
#define typeof_strip_qual(expr)                                                                                       \
    typeof(__builtin_choose_expr(                                                                                     \
        __builtin_types_compatible_p(typeof(expr), bool) || __builtin_types_compatible_p(typeof(expr), const bool) || \
 ...
 */
#define qatomic_read__nocheck(ptr) __atomic_load_n(ptr, __ATOMIC_RELAXED)
 ...
 */
#ifdef QEMU_SANITIZE_THREAD
#define qatomic_rcu_read__nocheck(ptr, valptr) __atomic_load(ptr, valptr, __ATOMIC_CONSUME);
#else
...
#define qatomic_store_release(ptr, i)                       \
    do {                                                    \
        qemu_build_assert(sizeof(*ptr) <= ATOMIC_REG_SIZE); \
        __atomic_store_n(ptr, i, __ATOMIC_RELEASE);         \
    } while (0)
...
#define qatomic_xchg__nocheck(ptr, i) ({ __atomic_exchange_n(ptr, (i), __ATOMIC_SEQ_CST); })
...
#define qatomic_cmpxchg__nocheck(ptr, old, new)                                                         \
    ({                                                                                                  \
        ...
        (void) __atomic_compare_exchange_n(ptr, &_old, new, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST); \
        _old;                                                                                           \
    })
 ...
 * the same semantics.
 */
#if !defined(QEMU_SANITIZE_THREAD) && (defined(__i386__) || defined(__x86_64__) || defined(__s390x__))
