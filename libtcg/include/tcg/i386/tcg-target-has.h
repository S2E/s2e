 ...
 */
#define have_avx512vl    ((cpuinfo & CPUINFO_AVX512VL) && (cpuinfo & CPUINFO_AVX512F))
...
#define TCG_TARGET_HAS_qemu_ldst_i128 (TCG_TARGET_REG_BITS == 64 && (cpuinfo & CPUINFO_ATOMIC_VMOVDQA))
...
#define TCG_TARGET_deposit_valid(type, ofs, len) \
    (((ofs) == 0 && ((len) == 8 || (len) == 16)) || (TCG_TARGET_REG_BITS == 32 && (ofs) == 8 && (len) == 8))
 ...
 */
static inline bool tcg_target_sextract_valid(TCGType type, unsigned ofs, unsigned len) {
    switch (ofs) {
        case 0:
            switch (len) {
                case 8:
...
static inline bool tcg_target_extract_valid(TCGType type, unsigned ofs, unsigned len) {
