...
void set_float_exception_flags(int val STATUS_PARAM) {
    STATUS_W(float_exception_flags, val);
}

void set_floatx80_rounding_precision(int val STATUS_PARAM) {
    STATUS_W(floatx80_rounding_precision, val);
}
...
static floatx80 roundAndPackFloatx80(int8 roundingPrecision, flag zSign, int32 zExp, uint64_t zSig0,
...
static void normalizeFloat128Subnormal(uint64_t aSig0, uint64_t aSig1, int32 *zExpPtr, uint64_t *zSig0Ptr,
                                       uint64_t *zSig1Ptr) {
    int8 shiftCount;
...
SINLINE float128 packFloat128(flag zSign, int32 zExp, uint64_t zSig0, uint64_t zSig1) {
    float128 z;
...
static float128 roundAndPackFloat128(flag zSign, int32 zExp, uint64_t zSig0, uint64_t zSig1,
...
static float16 packFloat16(flag zSign, int16 zExp, uint16_t zSig) {
    return make_float16((((uint32_t) zSign) << 15) + (((uint32_t) zExp) << 10) + zSig);
}
...
float64 float64_trunc_to_int(float64 a STATUS_PARAM) {
    ...
    STATUS_W(float_rounding_mode, float_round_to_zero);
    res = float64_round_to_int(a STATUS_VAR);
    STATUS_W(float_rounding_mode, oldmode);
    return res;
}
...
#define COMPARE(s, nan_exp)                                                                      \
    SINLINE int float##s##_compare_internal(float##s a, float##s b, int is_quiet STATUS_PARAM) { \
        ...
        if (((extractFloat##s##Exp(a) == nan_exp) && extractFloat##s##Frac(a)) ||                \
            ...
            if (!is_quiet || float##s##_is_signaling_nan(a) || float##s##_is_signaling_nan(b)) { \
                float_raise(float_flag_invalid STATUS_VAR);                                      \
            }                                                                                    \
...
SINLINE int floatx80_compare_internal(floatx80 a, floatx80 b, int is_quiet STATUS_PARAM) {
    flag aSign, bSign;

    if (((extractFloatx80Exp(a) == 0x7fff) && (extractFloatx80Frac(a) << 1)) ||
        ((extractFloatx80Exp(b) == 0x7fff) && (extractFloatx80Frac(b) << 1))) {
        if (!is_quiet || floatx80_is_signaling_nan(a) || floatx80_is_signaling_nan(b)) {
            float_raise(float_flag_invalid STATUS_VAR);
        }
        return float_relation_unordered;
    }
...
SINLINE int float128_compare_internal(float128 a, float128 b, int is_quiet STATUS_PARAM) {
    flag aSign, bSign;

    if (((extractFloat128Exp(a) == 0x7fff) && (extractFloat128Frac0(a) | extractFloat128Frac1(a))) ||
        ((extractFloat128Exp(b) == 0x7fff) && (extractFloat128Frac0(b) | extractFloat128Frac1(b)))) {
        if (!is_quiet || float128_is_signaling_nan(a) || float128_is_signaling_nan(b)) {
            float_raise(float_flag_invalid STATUS_VAR);
        }
        return float_relation_unordered;
    }
...
#define MINMAX(s, nan_exp)                                                               \
    SINLINE float##s float##s##_minmax(float##s a, float##s b, int ismin STATUS_PARAM) { \
        flag aSign, bSign;                                                               \
        uint##s##_t av, bv;                                                              \
        a = float##s##_squash_input_denormal(a STATUS_VAR);                              \
        b = float##s##_squash_input_denormal(b STATUS_VAR);                              \
