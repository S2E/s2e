...
//    switch (reg - nregs) {
//        case 0:
//            stl_p(buf, env->vfp.xregs[ARM_VFP_FPSID]);
//            return 4;
//        case 1:
//            stl_p(buf, env->vfp.xregs[ARM_VFP_FPSCR]);
//            return 4;
//        case 2:
//            stl_p(buf, env->vfp.xregs[ARM_VFP_FPEXC]);
//            return 4;
//    }
//    return 0;
//}
...
//            return 4;
//        case 1:
//            env->vfp.xregs[ARM_VFP_FPSCR] = ldl_p(buf);
//            return 4;
//        case 2:
...
static const struct arm_cpu_t arm_cpu_names[] = {{ARM_CPUID_ARM926, "arm926"},
        ...
        case EXCP_SWI:
            if (semihosting_enabled) {
                ...
                if (env->thumb) {
                    ...
                    mask = arm_ldl_code(env, env->regs[15] - 4, env->bswap_code) & 0xffffff;
                }
...
/* Check section/page access permissions.
   ...
   permitted.  */
static inline int check_ap(CPUARMState *env, int ap, int domain_prot, int access_type, int is_user) {
    int prot_ro;
...
static int get_phys_addr_v5(CPUARMState *env, uint32_t address, int access_type, int is_user, uint32_t *phys_ptr,
...
static int get_phys_addr_v6(CPUARMState *env, uint32_t address, int access_type, int is_user, uint32_t *phys_ptr,
                            int *prot, target_ulong *page_size) {
    int code;
    uint32_t table;
    uint32_t desc;
...
static int get_phys_addr_mpu(CPUARMState *env, uint32_t address, int access_type, int is_user, uint32_t *phys_ptr,
                             int *prot) {
    int n;
    uint32_t mask;
    uint32_t base;
...
static inline int get_phys_addr(CPUARMState *env, uint32_t address, int access_type, int is_user, uint32_t *phys_ptr,
                                int *prot, target_ulong *page_size) {
...
int cpu_arm_handle_mmu_fault(CPUARMState *env, target_ulong address, int access_type, int mmu_idx) {
    uint32_t phys_addr;
...
void HELPER(set_cp15)(CPUARMState *env, uint32_t insn, uint32_t val) {
    ...
    switch ((insn >> 16) & 0xf) {
        ...
        case 1: /* System configuration.  */
            if (arm_feature(env, ARM_FEATURE_V7) && op1 == 0 && crm == 1 && op2 == 0) {
                env->cp15.c1_scr = val;
                break;
            }
            if (arm_feature(env, ARM_FEATURE_OMAPCP))
                ...
                switch (crm) {
                    ...
                    case 8: {
                        ...
                        if (ret == 0) {
                            ...
                            env->cp15.c7_par =
                                ((ret & (10 << 1)) >> 5) | ((ret & (12 << 1)) >> 6) | ((ret & 0xf) << 1) | 1;
                        }
                        break;
                    }
                }
...
#define ADD16(a, b, n) RESULT(((int32_t)(int16_t)(a) + (int32_t)(int16_t)(b)) >> 1, n, 16)
#define SUB16(a, b, n) RESULT(((int32_t)(int16_t)(a) - (int32_t)(int16_t)(b)) >> 1, n, 16)
#define ADD8(a, b, n) RESULT(((int32_t)(int8_t)(a) + (int32_t)(int8_t)(b)) >> 1, n, 8)
#define SUB8(a, b, n) RESULT(((int32_t)(int8_t)(a) - (int32_t)(int8_t)(b)) >> 1, n, 8)
#define PFX sh

#include "op_addsub.h"

/* Halved unsigned arithmetic.  */
#define ADD16(a, b, n) RESULT(((uint32_t)(uint16_t)(a) + (uint32_t)(uint16_t)(b)) >> 1, n, 16)
#define SUB16(a, b, n) RESULT(((uint32_t)(uint16_t)(a) - (uint32_t)(uint16_t)(b)) >> 1, n, 16)
#define ADD8(a, b, n) RESULT(((uint32_t)(uint8_t)(a) + (uint32_t)(uint8_t)(b)) >> 1, n, 8)
#define SUB8(a, b, n) RESULT(((uint32_t)(uint8_t)(a) - (uint32_t)(uint8_t)(b)) >> 1, n, 8)
#define PFX uh
...
float32 HELPER(rsqrte_f32)(float32 a, CPUARMState *env) {
    ...
    if ((val & 0x800000) == 0) {
        f64 =
            make_float64(((uint64_t)(val & 0x80000000) << 32) | (0x3feULL << 52) | ((uint64_t)(val & 0x7fffff) << 29));
    } else {
        f64 =
            make_float64(((uint64_t)(val & 0x80000000) << 32) | (0x3fdULL << 52) | ((uint64_t)(val & 0x7fffff) << 29));
    }
