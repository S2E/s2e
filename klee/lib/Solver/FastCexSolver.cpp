...
public:
    ...
    ValueRange binaryAnd(const ValueRange &b) const {
        ...
        if (isFixed() && b.isFixed()) {
            ...
            return ValueRange(minAND(m_min, m_max, b.m_min, b.m_max), maxAND(m_min, m_max, b.m_min, b.m_max));
        }
    }
    ...
    ValueRange binaryOr(ValueRange b) const {
        ...
        if (isFixed() && b.isFixed()) {
            ...
            return ValueRange(minOR(m_min, m_max, b.m_min, b.m_max), maxOR(m_min, m_max, b.m_min, b.m_max));
        }
    }
    ...
    int64_t minSigned(unsigned bits) const {
        assert((m_min >> bits) == 0 && (m_max >> bits) == 0 && "range is outside given number of bits");
    ...
    void propogatePossibleValues(ref<Expr> e, CexValueData range) {
        ...
        switch (e->getKind()) {
            ...
            case Expr::Concat: {
                ...
                propogatePossibleValues(ce->getKid(0), range.extract(LSBWidth, LSBWidth + MSBWidth));
            ...
            case Expr::SExt: {
                ...
                ValueRange output = range.set_difference(ValueRange(
                    1 << (inBits - 1), (bits64::maxValueOfNBits(outBits) - bits64::maxValueOfNBits(inBits - 1) - 1)));
                    ...
                    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft())) {
                        ...
                        if (CE->getWidth() <= 64) {
                            ...
                            if (range.min()) {
                                ...
                                CexValueData range;
                                if (value == 0) {
                                    range = CexValueData(1, bits64::maxValueOfNBits(CE->getWidth()));
                                } else {
            ...
            case Expr::Ult: {
                ...
                if (range.isFixed()) {
                    ...
                    if (left.isFixed()) {
                        if (range.min()) {
                            propogatePossibleValues(be->getRight(), CexValueData(left.min() + 1, maxValue));
                        } else {
                        ...
                        if (range.min()) {
                            ...
                            propogatePossibleValues(be->getLeft(), CexValueData(right.min(), maxValue));
                        }
                    } else {
                        // XXX ???
                    }
            ...
            case Expr::Ule: {
                ...
                if (range.isFixed()) {
                    ...
                    if (left.isFixed()) {
                        if (range.min()) {
                            propogatePossibleValues(be->getRight(), CexValueData(left.min(), maxValue));
                        } else {
                        ...
                        if (range.min()) {
                            ...
                            propogatePossibleValues(be->getLeft(), CexValueData(right.min() + 1, maxValue));
                        }
                    } else {
                        // XXX ???
                    }
            ...
            case Expr::Read: {
                ...
                if (index.isFixed()) {
                    if (array->isConstantArray()) {
                        // Verify the range.
                        propogateExactValues(array->getConstantValues()[index.min()], range);
                    } else {
...
IncompleteSolver::PartialValidity FastCexSolver::computeTruth(const Query &query) {
    CexData cd;
