...
public:
    ...
    template <unsigned M> void copy(const NodeBase<T1, T2, M> &Other, unsigned i, unsigned j, unsigned Count) {
...
void adjustSiblingSizes(NodeT *Node[], unsigned Nodes, unsigned CurSize[], const unsigned NewSize[]) {
    ...
    for (int n = Nodes - 1; n; --n) {
        if (CurSize[n] == NewSize[n])
            continue;
        for (int m = n - 1; m != -1; --m) {
            int d = Node[n]->adjustFromLeftSib(CurSize[n], *Node[m], CurSize[m], NewSize[n] - CurSize[n]);
            CurSize[m] -= d;
            CurSize[n] += d;
    ...
    for (unsigned n = 0; n != Nodes - 1; ++n) {
        ...
        for (unsigned m = n + 1; m != Nodes; ++m) {
            int d = Node[m]->adjustFromLeftSib(CurSize[m], *Node[n], CurSize[n], CurSize[n] - NewSize[n]);
            CurSize[m] += d;
            CurSize[n] -= d;
            // Keep going if the current node was exhausted.
            if (CurSize[n] >= NewSize[n])
...
IdxPair distribute(unsigned Nodes, unsigned Elements, unsigned Capacity, const unsigned *CurSize, unsigned NewSize[],
...
template <typename KeyT, typename ValT> struct NodeSizer {
    enum {
        ...
        DesiredLeafSize = DesiredNodeBytes / static_cast<unsigned>(2 * sizeof(KeyT) + sizeof(ValT)),
        MinLeafSize = 3,
    ...
    enum {
        ...
        BranchSize = AllocBytes / static_cast<unsigned>(sizeof(KeyT) + sizeof(void *))
    };
...
public:
    ...
    unsigned findFrom(unsigned i, unsigned Size, KeyT x) const {
        ...
        assert((i == 0 || Traits::stopLess(stop(i - 1), x)) && "Index is past the needed point");
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
unsigned LeafNode<KeyT, ValT, N, Traits>::insertFrom(unsigned &Pos, unsigned Size, KeyT a, KeyT b, ValT y) {
    unsigned i = Pos;
...
public:
    ...
    unsigned findFrom(unsigned i, unsigned Size, KeyT x) const {
        assert(i <= Size && Size <= N && "Bad indices");
        assert((i == 0 || Traits::stopLess(stop(i - 1), x)) && "Index to findFrom is past the needed point");
        while (i != Size && Traits::stopLess(stop(i), x))
            ++i;
        return i;
    }
...
class Path {
    ...
    struct Entry {
        ...
        Entry(void *Node, unsigned Size, unsigned Offset) : node(Node), size(Size), offset(Offset) {
        }

        Entry(NodeRef Node, unsigned Offset) : node(&Node.subtree(0)), size(Node.size()), offset(Offset) {
        }
...
template <typename KeyT, typename ValT, unsigned N = IntervalMapImpl::NodeSizer<KeyT, ValT>::LeafSize,
...
class IntervalMap {
    ...
    using Branch = IntervalMapImpl::BranchNode<KeyT, ValT, Sizer::BranchSize, Traits>;
    ...
    enum {
        DesiredRootBranchCap = (sizeof(RootLeaf) - sizeof(KeyT)) / (sizeof(KeyT) + sizeof(IntervalMapImpl::NodeRef)),
        RootBranchCap = DesiredRootBranchCap ? DesiredRootBranchCap : 1
    };

    using RootBranch = IntervalMapImpl::BranchNode<KeyT, ValT, RootBranchCap, Traits>;
...
private:
    ...
    void visitNodes(void (IntervalMap::*f)(IntervalMapImpl::NodeRef, unsigned Level));
    void deleteNode(IntervalMapImpl::NodeRef Node, unsigned Level);

public:
    explicit IntervalMap(Allocator &a) : height(0), rootSize(0), allocator(a) {
        assert((uintptr_t(&data) & (alignof(RootLeaf) - 1)) == 0 && "Insufficient alignment");
        new (&rootLeaf()) RootLeaf();
    }
...
/// branched root.
template <typename KeyT, typename ValT, unsigned N, typename Traits>
ValT IntervalMap<KeyT, ValT, N, Traits>::treeSafeLookup(KeyT x, ValT NotFound) const {
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
IntervalMapImpl::IdxPair IntervalMap<KeyT, ValT, N, Traits>::branchRoot(unsigned Position) {
    ...
    if (Nodes == 1)
        ...
        NewOffset = distribute(Nodes, rootSize, Leaf::Capacity, nullptr, size, Position, true);
...
// Return the new (root offset, node offset) corresponding to Position.
template <typename KeyT, typename ValT, unsigned N, typename Traits>
IntervalMapImpl::IdxPair IntervalMap<KeyT, ValT, N, Traits>::splitRoot(unsigned Position) {
    using namespace IntervalMapImpl;
    // How many external leaf nodes to hold RootBranch+1?
    const unsigned Nodes = (unsigned) RootBranch::Capacity / (unsigned) Branch::Capacity + 1;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void IntervalMap<KeyT, ValT, N, Traits>::visitNodes(void (IntervalMap::*f)(IntervalMapImpl::NodeRef, unsigned Height)) {
    if (!branched())
        return;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void IntervalMap<KeyT, ValT, N, Traits>::deleteNode(IntervalMapImpl::NodeRef Node, unsigned Level) {
    if (Level)
...
template <typename KeyT, typename ValT, unsigned N, typename Traits> void IntervalMap<KeyT, ValT, N, Traits>::clear() {
    if (branched()) {
...
public:
    ...
    void advanceTo(KeyT x) {
        ...
        if (branched())
            ...
            path.leafOffset() = map->rootLeaf().findFrom(path.leafOffset(), map->rootSize, x);
    }
};
    ...
    if (path.height()) {
        for (unsigned l = path.height() - 1; l; --l) {
            if (!Traits::stopLess(path.node<Branch>(l).stop(path.offset(l)), x)) {
                ...
                path.offset(l + 1) = path.node<Branch>(l + 1).safeFind(path.offset(l + 1), x);
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool IntervalMap<KeyT, ValT, N, Traits>::iterator::canCoalesceLeft(KeyT Start, ValT Value) {
    using namespace IntervalMapImpl;
    Path &P = this->path;
    if (!this->branched()) {
        unsigned i = P.leafOffset();
        RootLeaf &Node = P.leaf<RootLeaf>();
        return i && Node.value(i - 1) == Value && Traits::adjacent(Node.stop(i - 1), Start);
    }
    // Branched.
...
/// @return True when updating the current interval would enable coalescing.
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool IntervalMap<KeyT, ValT, N, Traits>::iterator::canCoalesceRight(KeyT Stop, ValT Value) {
    using namespace IntervalMapImpl;
    Path &P = this->path;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void IntervalMap<KeyT, ValT, N, Traits>::iterator::setNodeStop(unsigned Level, KeyT Stop) {
...
void IntervalMap<KeyT, ValT, N, Traits>::iterator::setStop(KeyT b) {
    ...
    if (Traits::startLess(b, this->stop()) || !canCoalesceRight(b, this->value())) {
        setStopUnchecked(b);
        return;
    }
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool IntervalMap<KeyT, ValT, N, Traits>::iterator::insertNode(unsigned Level, IntervalMapImpl::NodeRef Node,
                                                              KeyT Stop) {
...
// insert
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void IntervalMap<KeyT, ValT, N, Traits>::iterator::insert(KeyT a, KeyT b, ValT y) {
    if (this->branched())
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void IntervalMap<KeyT, ValT, N, Traits>::iterator::treeInsert(KeyT a, KeyT b, ValT y) {
    using namespace IntervalMapImpl;
    Path &P = this->path;
    ...
    if (P.leafOffset() == 0 && Traits::startLess(a, P.leaf<Leaf>().start(0))) {
        ...
        if (NodeRef Sib = P.getLeftSibling(P.height())) {
            ...
            if (SibLeaf.value(SibOfs) == y && Traits::adjacent(SibLeaf.stop(SibOfs), a)) {
...
bool IntervalMap<KeyT, ValT, N, Traits>::iterator::overflow(unsigned Level) {
    ...
    unsigned NewSize[4];
    IdxPair NewOffset = distribute(Nodes, Elements, NodeT::Capacity, CurSize, NewSize, Offset, true);
...
public:
    ...
    IntervalMapOverlaps(const MapA &a, const MapB &b)
        : posA(b.empty() ? a.end() : a.find(b.start())), posB(posA.valid() ? b.find(posA.start()) : b.end()) {
        advance();
    }
