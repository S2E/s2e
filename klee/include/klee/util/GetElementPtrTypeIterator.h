...
public:
    ...
    generic_gep_type_iterator &operator++() { // Preincrement
        if (isa<llvm::StructType>(CurTy) || isa<llvm::ArrayType>(CurTy) || isa<llvm::VectorType>(CurTy)) {
...
inline gep_type_iterator gep_type_begin(const llvm::User *GEP) {
    return gep_type_iterator::begin(GEP->getOperand(0)->getType(), GEP->op_begin() + 1);
}
...
template <typename ItTy> inline generic_gep_type_iterator<ItTy> gep_type_begin(llvm::Type *Op0, ItTy I, ItTy E) {
    return generic_gep_type_iterator<ItTy>::begin(Op0, I);
}

template <typename ItTy> inline generic_gep_type_iterator<ItTy> gep_type_end(llvm::Type *Op0, ItTy I, ItTy E) {
